                  ...
                  @foreach($orders as $order)
                    ...
                    <a href="#" class="group">
                      <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">
                        <img src="{{ url('public/Image/'.$order->photo) }}" alt="" class="h-48 w-50 object-cover object-center group-hover:opacity-75">
                      </div>
                      <div class="col-xs-12 col-sm-12 col-md-12 text-center">
                      <button type="submit" class="btn btn-blue"></button></div>
                      <h3 class="mt-2 text-sm text-gray-700">name: {{$order->name}}</h3>
                      <h3 class="mt-2 text-sm text-gray-700">Cost: {{$order->cost}}</h3>
                      <h3 class="mt-2 text-sm text-green-700">Table no: {{$order->table_no}}</h3>


                              <!-- edit & delete button -->
                        <div class="px-4 py-3 text-center sm:px-6 flex flex-row-1 gap-12">
                          <form method="POST" action="/staff/order/{{ $order->id }}/edit">
                            @csrf
                            @method('GET')
                          ...
                          <form method="POST" action="/staff/order/{{ $order->id }}">
                            @csrf
                            @method('DELETE')
