...
<div class="flex flex-1 flex-col md:pl-64">
  ...
  <div class="mx-auto max-w-2xl py-8 px-4 sm:py-8 sm:px-6 lg:max-w-7xl lg:px-8">
    ...
    @foreach($staffteam as $staffteam)



      <a href="#" class="group">
        <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">
          <img src="{{ url('public/Image/'.$staffteam->photo) }}" alt="" class="h-48 w-50 object-cover object-center group-hover:opacity-75">
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <button type="submit" class="btn btn-blue"></button></div>
        <h3 class="mt-2 text-sm text-gray-700">Name: {{$staffteam->name}}</h3>
        <!-- <h3 class="mt-2 text-sm text-green-900">Status: {{$staffteam->status}}</h3> -->
        <h3 class="mt-2 text-sm text-green-900">Occupation: {{$staffteam->occupation}}</h3>
