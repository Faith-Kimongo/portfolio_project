  ...
  <div class="mx-auto max-w-2xl py-8 px-4 sm:py-8 sm:px-6 lg:max-w-7xl lg:px-8">
    ...
    <a href="#" class="group">
        <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">
          ...
          <img src="{{ url('public/category/'.$category->photo) }}" alt="" class="h-48 w-50 object-cover object-center group-hover:opacity-75">
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <button type="submit" class="btn btn-blue"></button></div>
        <h3 class="mt-2 text-sm text-gray-700">{{$category->name}}</h3>

        <!-- edit & delete button -->
        <div class="px-4 py-3 text-center sm:px-6 flex flex-row-1 gap-12">
          <form method="POST" action="/management/category/{{ $category->id }}/edit">
            @csrf
            @method('GET')
