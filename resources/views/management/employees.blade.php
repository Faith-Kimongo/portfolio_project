      ...
      <div class="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
        ...
        <nav aria-label="Sidebar" class="sticky top-4 divide-y divide-gray-300">
            <div class="space-y-1 pb-8">
              ...
              <a href="/management/mgtcalendar" class="bg-gray-200 text-gray-900 group flex items-center rounded-md px-3 py-2 text-sm font-medium" aria-current="page">
    ...
    @foreach($employees as $employee)
      ...
      <a href="#" class="group">
        <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">
          <img src="{{ url('public/Image/'.$employee->photo) }}" alt="" class="h-48 w-50 object-cover object-center group-hover:opacity-75">
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <button type="submit" class="btn btn-blue"></button></div>
        <h3 class="mt-2 text-sm text-gray-700">Name: {{$employee->name}}</h3>
        <h3 class="mt-2 text-sm text-green-900">Occupation: {{$employee->occupation}}</h3>
        <h3 class="mt-2 text-sm text-green-900">Availability: {{$employee->availability}}</h3>

                <!-- edit & delete button -->
          <div class="px-4 py-3 text-center sm:px-6 flex flex-row-1 gap-12">
            <form method="POST" action="/management/employees/{{ $employee->id }}/edit">
              @csrf
              @method('GET')
            ...
            <form method="POST" action="/management/employees/{{ $employee->id }}">
              @csrf
              @method('DELETE')
