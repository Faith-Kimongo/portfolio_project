...
<div class="flex flex-1 flex-col md:pl-64">
    ...
    <main class="flex-1">
      <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h1 class="text-2xl font-semibold text-gray-900">OUR PRODUCTS</h1>
        </div>
        <div class="text-right mr-2">
        <a href="/management/mgtproducts/create">
         <button type="submit"  class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 inline-flex justify-center rounded-md border border-transparent bg-gray-600 py-3 px-6 text-base font-medium text-white hover:text-black shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">Add Products</button></a>
          </div>
  ...
  <div class="mx-auto max-w-2xl py-8 px-4 sm:py-8 sm:px-6 lg:max-w-7xl lg:px-8">
    ...
    @foreach($mgtproducts as $mgtproduct) 
    

    <a href="#" class="group">
        <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8">
          <!-- url where image is stored as file. -->
          <img src="{{ url('public/Image/'.$mgtproduct->photo) }}" alt="" class="h-48 w-50 object-cover object-center group-hover:opacity-75">
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <button type="submit" class="btn btn-blue"></button></div>
        <h3 class="mt-2 text-sm text-gray-700">Name: {{$mgtproduct->name}}</h3>
        <h3 class="mt-2 text-sm text-green-900">Table_no: {{$mgtproduct->Table_no}}</h3>
        <h3 class="mt-2 text-sm text-gray-700">Cost: {{$mgtproduct->cost}}</h3>
        {{-- <h3 class="mt-2 text-sm text-gray-700">{{$mgtproduct->duration}}</h3> --}}

        <!-- edit & delete button -->
        <div class="px-4 py-3 text-center sm:px-6 flex flex-row-1 gap-12">
          <form method="POST" action="/management/mgtproducts/{{ $mgtproduct->id }}/edit">
            @csrf
            @method('GET')
          ...
          <form method="POST" action="/management/mgtproducts/{{ $mgtproduct->id }}">
            @csrf
            @method('DELETE')
